...
int main()
{
    ...
    // 2. Utworzenie instancji regulatora
    Regulator reg_A (30, 10, 5, 1.0, 1.0, 0.0, 1.0, -0.2, 0.2, obj_s_A);

    // 3. Stworzenie obiektu sterowanego (takiego samego jak ten generujący s)
    ObiektSymulowany obj_A (0.8351, 0.2713, 2);

    // 4. Stworzenie instancji symulacji
    Symulacja sym_A (100, 1.0, obj_A, reg_A);

    // 5. Uruchomienie pętli regulacji
    sym_A.uruchom();
    ...
    // 2. Utworzenie instancji regulatora
    Regulator reg_B (30, 10, 5, 1.0, 1.0, 0.0, 1.0, -0.2, 0.2, obj_s_B);
